...
import { getAbout, getAcademicService, getAwards, getNews, isLocale, locales } from '@/lib/siteContent';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><AcademicHome locale={locale} about={getAbout(locale)} news={getNews(locale)} awards={getAwards(locale)} academicService={getAcademicService(locale)} /></>;
}
